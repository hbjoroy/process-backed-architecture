...
$response = Invoke-RestMethod -Uri "$baseURL/$paymentService/$paymentProduct/$paymentId/status" -Method Get -ContentType "application/json"

# Display the response from the webapi
Write-Output "Status Get $response"

$response = Invoke-RestMethod -Uri "$baseURL/$paymentService/$paymentProduct/$paymentId" -Method Delete -ContentType "application/json"
